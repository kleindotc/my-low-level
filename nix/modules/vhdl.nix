...
let
  ...
  defaultPackageText = if pkgs.stdenv.isDarwin then "pkgs.ghdl-llvm" else "pkgs.ghdl-gcc";
in
{
  options.languages.vhdl = {
    ...
    compiler = lib.mkOption {
      ...
      default = if pkgs.stdenv.isDarwin then "ghdl-llvm" else "ghdl-gcc";
      description = ''
    ...
    lsp = {
      enable = lib.mkEnableOption "VHDL Language Server." // {
        default = true;
      };
      package = lib.mkOption {
        type = lib.types.package;
