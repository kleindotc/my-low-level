...
let
  ...
  isCrossCompile = builtins.elem cfg.type [
    "arm"
    "riscv"
  ];
  ...
  isValidArchMapping =
    builtins.hasAttr cfg.type archMapping && builtins.elem cfg.arch archMapping.${cfg.type};
  ...
  crossPkgs = if isCrossCompile && isValidArchMapping then crossArchMap.${cfg.arch} or { } else { };

  defaultPackage =
    if !isValidArchMapping then
      throw invalidArchMappingMsg
    else if isCrossCompile then
      crossPkgs.stdenv.cc
    else
      asmTargetPkgs.${cfg.type};

  defaultBinutils = if isCrossCompile && isValidArchMapping then crossPkgs.binutils or null else null;

  rvvmRequiresRiscv = !(cfg.emulator.enable && cfg.emulator.type == "rvvm" && cfg.type != "riscv");
  ...
  asmLspAssembler =
    if cfg.lsp.config.assembler != null then
      cfg.lsp.config.assembler
    else
      asmLspAssemblerMap.${cfg.type};

  asmLspInstructionSet =
    if cfg.lsp.config.instructionSet != null then
      cfg.lsp.config.instructionSet
    else
      asmLspInstructionSetMap.${cfg.arch} or (throw ''
  ...
  asmLspDiagnostics =
    if cfg.lsp.config.diagnostics != null then cfg.lsp.config.diagnostics else asmLspDiagnosticsDefault;

  asmLspCompiler =
    if asmLspDiagnostics && isCrossCompile && isValidArchMapping then "${cfg.package}/bin/cc" else null;
...
in
{
  options.languages.assembly = {
    ...
    arch = lib.mkOption {
      ...
      default =
        if cfg.type == "riscv" then
          "riscv64"
        else if cfg.type == "arm" then
          "aarch64"
        else
          "x86_64";
    ...
    lsp = {
      enable = lib.mkEnableOption "Assembly Language Server." // {
        default = cfg.enable;
      };
      package = lib.mkOption {
        type = lib.types.package;
    ...
    emulator = {
      enable = lib.mkEnableOption "System Emulator." // {
      ...
      type = lib.mkOption {
        ...
        default = if cfg.type == "riscv" then "rvvm" else "qemu";
